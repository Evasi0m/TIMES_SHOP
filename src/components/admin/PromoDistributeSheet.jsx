  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end justify-center lg:items-center">
      ...
      <div
        role="dialog"
        aria-modal="true"
        className="relative z-10 w-full max-w-lg rounded-t-2xl bg-surface-strong p-4 shadow-high lg:rounded-2xl lg:p-6"
      >
          ...
          {mode === 'targeted' && (
            <div className="space-y-3">
              ...
              <div className="max-h-40 space-y-2 overflow-y-auto rounded-lg border border-hairline p-2">
                {customers.map((c) => (
                  <label key={c.id} className="flex cursor-pointer items-center gap-2 rounded-md p-2 hover:bg-surface-soft">
                    <input
                      type="checkbox"
