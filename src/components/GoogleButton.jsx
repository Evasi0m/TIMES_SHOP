import { useAuth } from '../context/AuthContext.jsx';
import { useToast } from '../context/ToastContext.jsx';
import { mapError } from '../lib/error-map.js';

// Google sign-in UI is always present (per SHOP_REQUIREMENTS Q11). It is disabled
// with a hint until VITE_GOOGLE_REDIRECT_URL + Supabase OAuth client are configured.
export default function GoogleButton() {
  const { signInWithGoogle, googleEnabled } = useAuth();
  const toast = useToast();

  async function handleClick() {
    try {
      await signInWithGoogle();
    } catch (err) {
      toast.error(mapError(err));
    }
  }

  return (
    <div className="mt-4">
      <div className="my-4 flex items-center gap-3 text-xs text-muted">
        <span className="h-px flex-1 bg-hairline" />
        หรือ
        <span className="h-px flex-1 bg-hairline" />
      </div>
      <button
        type="button"
        onClick={handleClick}
        disabled={!googleEnabled}
        title={googleEnabled ? undefined : 'ยังไม่ได้ตั้งค่า Google OAuth ใน Supabase'}
        className="btn-outline w-full"
      >
        <GoogleGlyph />
        เข้าสู่ระบบด้วย Google
      </button>
      {!googleEnabled && (
        <p className="mt-2 text-center text-[11px] text-muted-soft">
          ตั้งค่า Google Sign-in ใน Supabase (เร็วๆ นี้)
        </p>
      )}
    </div>
  );
}

function GoogleGlyph() {
  return (
    <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
      <path
        fill="#FFC107"
        d="M43.6 20.5H42V20H24v8h11.3c-1.6 4.7-6.1 8-11.3 8-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.9 1.2 8 3.1l5.7-5.7C34 6.1 29.3 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.3-.4-3.5z"
      />
      <path
        fill="#FF3D00"
        d="M6.3 14.7l6.6 4.8C14.7 16 19 13 24 13c3.1 0 5.9 1.2 8 3.1l5.7-5.7C34 6.1 29.3 4 24 4 16.3 4 9.7 8.3 6.3 14.7z"
      />
      <path
        fill="#4CAF50"
        d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2c-2 1.5-4.5 2.4-7.2 2.4-5.2 0-9.6-3.3-11.3-7.9l-6.5 5C9.6 39.6 16.2 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.2 4.2-4.1 5.6l6.2 5.2c-.4.4 6.6-4.8 6.6-14.8 0-1.3-.1-2.3-.4-3.5z"
      />
    </svg>
  );
}
