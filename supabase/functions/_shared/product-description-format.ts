...
export type ParsedSpec = { key: string; label: string; value: string };
...
export function formatSpecLineBreaks(text: string): string {
  let out = String(text ?? '');
...
function findSpecMarkers(text: string): Array<{ label: string; key: string; index: number; length: number }> {
  const markers: Array<{ label: string; key: string; index: number; length: number }> = [];
  for (const label of SPEC_FIELD_LABELS) {
    let from = 0;
...
export function parseDescriptionSpecs(text: string): ParsedSpec[] {
...
export function getSpecSummaryKeys(): string[] {
  return ['case_size', 'weight', 'water_resist', 'battery_life'];
}

export function pickSummarySpecs(specs: ParsedSpec[], limit = 4): ParsedSpec[] {
  const order = getSpecSummaryKeys();
  const picked: ParsedSpec[] = [];
